package bg.dr.chilly.currency.service.util;
...
public class CurrencyRateExcelReportWriter {
  ...
  public static ByteArrayInputStream writeExcelReport(List<CurrencyRateView> currencyRates)
    ...
    String[] COLUMNs = {"Id", "CreatedOn", "UpdatedOn", "Base", "Quote", "Name", "Rate",
        "ReverseRate", "CurrencyRateProvider", "ProviderCreatedOn"};
    try (
      ...
      for (CurrencyRateView view : currencyRates) {
        ...
        row.createCell(8).setCellValue(view.getCurrencyRateProvider().getName());

        cell = row.createCell(9);
        cell.setCellValue((map(view.getProviderCreatedOn())));
        cell.setCellStyle(dateStyle);
      }
      workbook.write(out);
